<template>
   ...
   <Popover class="relative w-full" v-slot="{ open  }">
        ...
        <PopoverPanel class="absolute z-10 mt-2 rounded-md h-80 overflow-y-auto bg-white dark:bg-neutral-900  ring-1 ring-slate-300/40 rounded-m">
            <!-- Search input -->
           <input v-model="searchQuery" class=" h-8 w-full bg-white dark:bg-neutral-900 ring-1 ring-slate-300/20  px-1 focus:outline-0"
...
const onTimeZoneSelect = (code:String, country:string) =>{
    ...
    languageStore.to_language = code.toString()
